...
use crate::mm_ref::barrier::{BarrierType, system_barrier};
use crate::mm_ref::mapping::MappingBackingMemory;
use crate::mm_ref::memory::MemoryObject;
use crate::mm_ref::memory_accessor::{MemoryAccessor, TaskMemoryAccessor};
use crate::mm_ref::private_anonymous_memory_manager::PrivateAnonymousMemoryManager;
use crate::mm_ref::{
...
use m6_starnix_std::collections::HashMap;
use m6_starnix_std::mem::MaybeUninit;
use m6_starnix_std::ops::{Deref, DerefMut, Range, RangeBounds};
use m6_starnix_std::sync::{Arc, LazyLock, Weak};
...
// Unified address spaces mean the Starnix kernel and its Linux processes share
// one address space, so user memory can be touched with direct `usercopy`
// (Zircon's restricted-mode model). M6 runs each Linux process in a SEPARATE
// VSpace (`restricted_enter` switches EL0 to a distinct vspace_cptr), so a
// direct dereference of a user address from svc-starnix faults — user memory
// must be reached through the VMO/`frame_read`+`frame_write` path
// (`syscall_read_memory`/`syscall_write_memory`). Hence M6 is always
// non-unified. (`cfg!(not(test))` upstream; left here for the test builds.)
pub(crate) const UNIFIED_ASPACES_ENABLED: bool = false;
...
//
// M6 bring-up: the stack mapping is committed EAGERLY by the zx-shim `Vmar::map`
// (no demand paging yet), and every frame consumes a slot in the (radix-12, 4096
// slot) cnode shared with the m6-std heap. A 512 MiB stack would need 131072
// frames — far over budget and overlapping the heap. 256 KiB (64 pages) is ample
// for the static hello-world and keeps the eager mapping tiny. Raising this needs
...
impl ReleasedMappings {
    ...
    fn finalize(&mut self, mm_state: RwLockWriteGuard<'_, MemoryManagerState>) {
        ...
        // in `DirEntry`'s `drop`.
        m6_starnix_std::mem::drop(mm_state);
        m6_starnix_std::mem::take(&mut self.doomed);
        m6_starnix_std::mem::take(&mut self.doomed_pins);
    }
}
...
impl MemoryManagerState {
    ...
    fn find_random_unused_range(
        ...
        // Pick a placement uniformly at random. Upstream uses the `rand` crate;
        // M6 draws from the zx-shim CPRNG (consistent with
        // `generate_random_offset_for_aslr`), avoiding a host-only dependency.
        let chosen_placement_idx = {
            let mut bytes = [0u8; core::mem::size_of::<usize>()];
            zx::cprng_draw(&mut bytes);
            usize::from_le_bytes(bytes) % possible_placements
        };
    ...
    fn remap_move(
        ...
        match self.get_mapping_backing(&src_mapping) {
            MappingBacking::PrivateAnonymous => {
                ...
                let length_to_move = m6_starnix_std::cmp::min(dst_length, src_length) as u64;
        ...
        L: LockBefore<ThreadGroupLimits>,
    {
        ...
        for (range, mapping) in self.mappings.range(start_addr..end_addr) {
            ...
            range.start = m6_starnix_std::cmp::max(range.start, start_addr);
            // Handle mappings that extend past the region to be locked.
            range.end = m6_starnix_std::cmp::min(range.end, end_addr);
    ...
    fn get_contiguous_mappings_at(
        ...
        let mut offset = 0;
        let result = m6_starnix_std::iter::from_fn(move || {
            if offset != length {
                if let Some((range, mapping)) = mappings.next() {
                    return match prev_range_end {
                        ...
                        _ => {
                            let mapping_length = m6_starnix_std::cmp::min(length, range.end - addr) - offset;
    ...
    fn read_memory<'a>(
        ...
        if bytes_read != bytes.len() {
            ...
            let bytes = unsafe {
                m6_starnix_std::slice::from_raw_parts_mut(bytes.as_mut_ptr() as *mut u8, bytes_read)
            };
            Ok(bytes)
        }
    }
    ...
    where
        L: LockBefore<ThreadGroupLimits>,
    {
        let rlimit_data = m6_starnix_std::cmp::min(
            PROGRAM_BREAK_LIMIT,
        ...
        match new_end.cmp(&old_end) {
            m6_starnix_std::cmp::Ordering::Less => {
            ...
            m6_starnix_std::cmp::Ordering::Greater => {
    ...
    fn set_mapping_name(
        ...
        for (mut range, mut mapping) in mappings_in_range {
            ...
            range.start = m6_starnix_std::cmp::max(range.start, addr);
            // Handle mappings that extend past the region to be named.
            range.end = m6_starnix_std::cmp::min(range.end, end);
...
impl MemoryManager {
    pub fn summarize(&self, summary: &mut crate::mm_ref::MappingSummary) {
...
impl MemoryManager {
        ...
        L: LockBefore<UserFaultInner>,
    {
        ...
        let trimmed_end = m6_starnix_std::cmp::min(first_populated, end_addr);
        ...
        L: LockBefore<UserFaultInner>,
    {
        self.populate_from_uffd(locked, dst_addr, length, userfault, |state, effective_length| {
            let mut buf = vec![m6_starnix_std::mem::MaybeUninit::uninit(); effective_length];
    ...
    pub fn exec(
        ...
        let (_old_mappings, user_vmar) = {
            ...
            (m6_starnix_std::mem::replace(&mut state.mappings, Default::default()), user_vmar)
        };
    ...
    pub fn handle_page_fault(
        ...
        if decoded.not_present {
            ...
            // M6: demand-page a registered-but-uncommitted lazy mapping. The
            // forked core maps non-populated regions without committing (Fuchsia
            // relies on its kernel to fault those pages in); on M6 the zx-shim
            // registers them and we commit the faulting page here. Mirrors the
            // growsdown resolution above — resolve the fault and retry.
            let vspace_cptr = self.state.read().user_vmar.vspace_cptr();
            match zx::mem_context::commit_fault_page(vspace_cptr, decoded.faulting_address) {
                Ok(true) => return ExceptionResult::Handled,
                Ok(false) => {}
                Err(_) => log_warn!("Error committing demand page"),
            }
        }
            ...
            let buf = unsafe {
                read_to_array(|buf| {
                    self.state.read().read_memory(futex_addr.into(), buf).map(|bytes_read| {
                        debug_assert_eq!(bytes_read.len(), m6_starnix_std::mem::size_of::<u32>())
                    })
                })
            }?;
...
fn write_map(
    ...
    match &map.name() {
        ...
        MappingNameRef::File(file) => {
            ...
            sink.write_iter(
                path.iter()
                    .flat_map(|b| if *b == b'\n' { b"\\012" } else { m6_starnix_std::slice::from_ref(b) })
                    .copied(),
            );
        }
...
fn generate_random_offset_for_aslr(arch_width: ArchWidth) -> usize {
    // M6 bring-up: ASLR offsets are sized for a 47-bit aspace; in M6's capped
    // 2 GiB restricted aspace they randomise the stack/mmap regions into
    // overlapping positions (eager mapping then fails with AlreadyMapped). Use a
    // deterministic zero offset so the layout is stable and non-overlapping.
    let _ = arch_width;
    return 0;
    ...
    let randomness = {
        ...
        let mut bytes = [0; m6_starnix_std::mem::size_of::<usize>()];
        zx::cprng_draw(&mut bytes);
...
mod tests {
    use super::*;
    use crate::mm_ref::memory_accessor::MemoryAccessorExt;
    use crate::mm_ref::syscalls::do_mmap;
    ...
    async fn test_read_object_partial() {
        ...
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            let items_read = ma
                .read_object_partial(items_ref, m6_starnix_std::mem::size_of::<Items>())
            ...
            assert_eq!(
                ma.read_object_partial(items_ref, m6_starnix_std::mem::size_of::<Items>() + 8),
                error!(EINVAL)
            );
    ...
    async fn test_snapshot_paged_memory() {
        ...
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            let thread = m6_starnix_std::thread::spawn(move || {
                loop {
                    let packet =
