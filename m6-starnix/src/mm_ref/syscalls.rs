...
use crate::mm_ref::debugger::notify_debugger_of_module_list;
use crate::mm_ref::{
...
pub fn sys_get_robust_list(
    ...
    current_task.write_object(user_len_ptr, &m6_starnix_std::mem::size_of::<robust_list_head>())?;
    Ok(())
}
...
pub fn sys_set_robust_list(
    ...
    if len != m6_starnix_std::mem::size_of::<robust_list_head>() {
        return error!(EINVAL);
    }
...
mod arch32 {
    ...
    use crate::mm_ref::syscalls::{UserAddress, sys_mmap};
    ...
    pub fn sys_arch32_set_robust_list(
        ...
        if len != m6_starnix_std::mem::size_of::<uapi::arch32::robust_list_head>() {
            return error!(EINVAL);
        }
...
mod tests {
    ...
    async fn test_munmap_many_mappings() {
        spawn_kernel_and_run(async |locked, current_task| {
            let mapped_addresses: Vec<_> = m6_starnix_std::iter::repeat_with(|| {
                map_memory(locked, &current_task, UserAddress::default(), *PAGE_SIZE)
            })
            .take(3)
            .collect();
