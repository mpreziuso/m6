...
use crate::mm_ref::{CompareExchangeResult, ProtectionFlags};
...
impl FutexWaiters {
    ...
    fn split_for_requeue(&mut self, count: usize) -> Self {
        let count = m6_starnix_std::cmp::min(count, self.0.len());
        let tail = self.0.split_off(count);
        let head = m6_starnix_std::mem::replace(&mut self.0, tail);
        FutexWaiters(head)
    }
}
