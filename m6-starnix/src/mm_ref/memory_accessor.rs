...
fn object_as_mut_bytes<T: FromBytes + Sized>(
    ...
    // are larger than u8.
    unsafe {
        m6_starnix_std::slice::from_raw_parts_mut(
            object.as_mut_ptr() as *mut MaybeUninit<u8>,
            m6_starnix_std::mem::size_of::<T>(),
        )
    }
}
...
fn slice_as_mut_bytes<T: FromBytes + Sized>(
    ...
    unsafe {
        m6_starnix_std::slice::from_raw_parts_mut(
            ...
            slice.len() * m6_starnix_std::mem::size_of::<T>(),
        )
    }
}
...
pub trait MemoryAccessorExt: MemoryAccessor {
        ...
        unsafe {
            read_to_object_as_bytes(|buf| {
                self.read_memory(user.addr(), buf)
                    .map(|bytes_read| debug_assert_eq!(bytes_read.len(), m6_starnix_std::mem::size_of::<T>()))
            })
        }
    }
    ...
    fn read_object_partial<T: FromBytes>(
        ...
        let full_size = m6_starnix_std::mem::size_of::<T>();
    ...
    fn read_objects<'a, T: FromBytes>(
        ...
        self.read_memory(user.addr(), slice_as_mut_bytes(objects)).map(|bytes_read| {
            debug_assert_eq!(bytes_read.len(), objects_len * m6_starnix_std::mem::size_of::<T>());
            // SAFETY: `T` implements `FromBytes` and all bytes have been initialized.
            unsafe {
                m6_starnix_std::slice::from_raw_parts_mut(bytes_read.as_mut_ptr() as *mut T, objects_len)
            }
        })
    }
    ...
    fn read_objects_to_smallvec<T: Clone + FromBytes, const N: usize>(
        ...
        if len > N {
            ...
            let buffer: MaybeUninit<[T; N]> = unsafe { m6_starnix_std::mem::transmute_copy(&buffer) };
    ...
    fn read_c_string_to_vec(
        ...
        let chunk_size = m6_starnix_std::cmp::min(*PAGE_SIZE as usize, max_size);
