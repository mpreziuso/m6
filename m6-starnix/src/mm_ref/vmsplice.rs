...
impl VmsplicePayloadSegment {
    ...
    fn truncate(&mut self, limit: usize) {
        ...
        // memory, etc.).
        self.length = m6_starnix_std::cmp::min(self.length, limit);
    }
    ...
    fn copy_to_user(&self, data: &mut dyn OutputBuffer) -> Result<usize, Errno> {
        let result = with_iovec_segments(data, |iovecs: &mut [syncio::zxio::zx_iovec]| {
            ...
            for iovec in iovecs {
                ...
                iovec.capacity = m6_starnix_std::cmp::min(iovec.capacity, length - copied);
                ...
                while let Some(segment) = &mut current_segment {
                    ...
                    let to_read = m6_starnix_std::cmp::min(segment.length, iovec.capacity - iovec_pos);
...
mod tests {
    ...
    async fn lifecycle() {
        spawn_kernel_and_run(async |_, current_task| {
            ...
            m6_starnix_std::mem::drop(payload);
