...
use crate::mm_ref::{MemoryManager, PAGE_SIZE, VMEX_RESOURCE, ZX_VM_SPECIFIC_OVERWRITE};
...
impl m6_starnix_std::cmp::Eq for MemoryObject {}

// Implemented manually as `MemoryMappedClock`'s mutex is not transparent to
// `PartialEq`.
impl m6_starnix_std::cmp::PartialEq for MemoryObject {
    ...
    pub fn map_in_vmar(
        ...
        match self {
            ...
            Self::MemoryMappedClock { utc_clock, .. } => {
                ...
                // M6: the shim's `try_new_without_unmap` is generic over the clock type
                // and can't infer the MappedClock timelines; a UTC clock maps the boot
                // timeline to the UTC timeline.
                let memory_mapped_clock =
                    MappedClock::<zx::BootTimeline, zx::UtcTimeline>::try_new_without_unmap(
                        &utc_clock,
                        vmar,
                        flags,
                        vmar_offset as u64,
