...
use crate::mm_ref::memory_manager::MemoryManagerState;
use crate::mm_ref::{
...
// REMOVED(fuchsia_inspect) use fuchsia_inspect::HistogramProperty;
...
//
// M6: upstream packs this to 24 bytes; the M6 shim types (`MappingBacking`,
// `MappingName`, `Access`) are currently wider, so the port lands at 32 bytes.
// Tightening these shim types back to the upstream layout is a follow-up
// optimisation; the size is functionally irrelevant for first light.
