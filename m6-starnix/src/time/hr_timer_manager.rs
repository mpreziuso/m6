...
// REMOVED(fidl) use fidl_fuchsia_time_alarms as fta;
// REMOVED(fuchsia_async) use fuchsia_async as fasync;
// REMOVED(fuchsia_inspect) use fuchsia_inspect::ArrayProperty;
// REMOVED(fuchsia_runtime) use fuchsia_runtime::UtcClock;
// REMOVED(fuchsia_trace) use fuchsia_trace as ftrace;
...
fn to_errno_with_log<T: m6_starnix_std::fmt::Debug>(v: T) -> Errno {
...
impl m6_starnix_std::fmt::Display for InspectHrTimerEvent {
...
impl m6_starnix_std::fmt::Display for TimerState {
    fn fmt(&self, f: &mut m6_starnix_std::fmt::Formatter<'_>) -> m6_starnix_std::fmt::Result {
...
async fn run_utc_timeline_monitor_internal(
    ...
    let utc_handle = m6_starnix_std::rc::Rc::new(utc_handle);
...
impl HrTimerManager {
    ...
    fn init_internal(
        ...
        let closure = async move |locked_and_task: LockedAndTask<'_>| {
            let current_thread = m6_starnix_std::thread::current();
