...
use m6_starnix_std::os::unix::thread::JoinHandleExt;
use m6_starnix_std::sync::Arc;
use m6_starnix_std::sync::mpsc::sync_channel;
    ...
    G: FnOnce(Result<ExitStatus, Error>) + Send + Sync + 'static,
{
    ...
    let result = m6_starnix_std::thread::Builder::new().name("user-thread".to_string()).spawn(move || {
        ...
        m6_starnix_std::mem::drop(receiver);
        ...
        if pre_run_result.is_err() {
            ...
            // releasables.
            m6_starnix_std::mem::drop(task_complete);
        } else {
    ...
    let join_handle = match result {
        ...
        Err(e) => {
            ...
            match e.kind() {
                m6_starnix_std::io::ErrorKind::WouldBlock => return error!(EAGAIN),
