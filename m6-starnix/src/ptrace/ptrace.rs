...
use crate::mm_ref::{IOVecPtr, MemoryAccessor, MemoryAccessorExt};
...
use m6_starnix_std::sync::atomic::Ordering;
                ...
                match target.load_stopped() {
                    StopState::SyscallEnterStopped => {
                        ...
                        info_len += memoffset::offset_of!(
                            ...
                            args
                        ) + m6_starnix_std::mem::size_of_val(&entry.args);
                    ...
                    StopState::SyscallExitStopped => {
                        ...
                        info_len += memoffset::offset_of!(
                            ...
                            is_error
                        ) + m6_starnix_std::mem::size_of_val(&exit.is_error);
...
impl ZombiePtracees {
    ...
    pub fn add(&mut self, pids: &mut PidTable, tid: tid_t, zombie: ZombieProcess) {
        if let m6_starnix_std::collections::btree_map::Entry::Vacant(entry) = self.zombies.entry(tid) {
    ...
    pub fn set_parent_of(
        ...
        match self.zombies.entry(tracee) {
            m6_starnix_std::collections::btree_map::Entry::Vacant(entry) => {
                if let Some(new_zombie) = new_zombie {
            ...
            m6_starnix_std::collections::btree_map::Entry::Occupied(mut entry) => {
    ...
    L: LockBefore<ThreadGroupLimits>,
{
    ...
    match request {
        ...
        PTRACE_SYSCALL => {
            tracee.trace_syscalls.store(true, m6_starnix_std::sync::atomic::Ordering::Relaxed);
            ptrace_cont(locked, &tracee, &data, false)?;
            return Ok(starnix_syscalls::SUCCESS);
        }
        PTRACE_DETACH => {
    ...
    match request {
        ...
        PTRACE_SETSIGMASK => {
            ...
            if addr.ptr() != m6_starnix_std::mem::size_of::<SigSet>() {
                return error!(EINVAL);
            }
        ...
        PTRACE_GET_SYSCALL_INFO => {
            if let Some(ptrace) = &state.ptrace {
                ...
                let len = m6_starnix_std::cmp::min(m6_starnix_std::mem::size_of::<ptrace_syscall_info>(), addr.ptr());
                // SAFETY: ptrace_syscall_info does not implement FromBytes/IntoBytes,
                // so this has to happen manually.
                let src = unsafe {
                    m6_starnix_std::slice::from_raw_parts(
...
pub fn ptrace_peekuser(
    ...
    if offset >= m6_starnix_std::mem::size_of::<user>() {
        return error!(EIO);
    }
