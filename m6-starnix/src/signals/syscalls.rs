...
pub fn sys_rt_sigaction(
    ...
    if user_action.is_arch32() && sigset_size == m6_starnix_std::mem::size_of::<uapi::arch32::sigset_t>() {
    ...
    if sigset_size != m6_starnix_std::mem::size_of::<uapi::sigset_t>() {
        return error!(EINVAL);
    }
...
pub fn sys_rt_sigpending(
    ...
    if sigset_size != m6_starnix_std::mem::size_of::<SigSet>() {
        return error!(EINVAL);
    }
...
pub fn sys_signalfd4(
    ...
    if mask_size != m6_starnix_std::mem::size_of::<SigSet>() {
        return error!(EINVAL);
    }
...
mod tests {
    use super::*;
    use crate::mm_ref::{MemoryAccessor, PAGE_SIZE};
    ...
    async fn test_sigaltstack() {
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            sys_sigaltstack(locked, &current_task, user_ss.into(), nullptr.into())
                .expect("failed to call sigaltstack");
            current_task
                .write_memory(addr, &[0u8; m6_starnix_std::mem::size_of::<sigaltstack>()])
        ...
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            assert_eq!(
                sys_rt_sigprocmask(
                    ...
                    set,
                    old_set,
                    m6_starnix_std::mem::size_of::<SigSet>() * 2
                ),
                error!(EINVAL)
            );
            assert_eq!(
        ...
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            assert_eq!(
                sys_rt_sigprocmask(
                    ...
                    set,
                    old_set,
                    m6_starnix_std::mem::size_of::<SigSet>()
                ),
                error!(EINVAL)
            );
        })
    ...
    async fn test_sigprocmask_null_set() {
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            current_task
                .write_memory(addr, &[0u8; m6_starnix_std::mem::size_of::<SigSet>()])
                .expect("failed to clear struct");
    ...
    async fn test_sigprocmask_setmask() {
        spawn_kernel_and_run(async |locked, current_task| {
            let addr = map_memory(locked, &current_task, UserAddress::default(), *PAGE_SIZE);
            current_task
                .write_memory(addr, &[0u8; m6_starnix_std::mem::size_of::<SigSet>() * 2])
                .expect("failed to clear struct");
            ...
            let old_addr_range = (addr + m6_starnix_std::mem::size_of::<SigSet>()).unwrap();
    ...
    async fn test_sigaction_old_value_set() {
        spawn_kernel_and_run(async |locked, current_task| {
            let addr = map_memory(locked, &current_task, UserAddress::default(), *PAGE_SIZE);
            current_task
                .write_memory(addr, &[0u8; m6_starnix_std::mem::size_of::<sigaction_t>()])
                .expect("failed to clear struct");

            let org_mask = SigSet::from(SIGHUP) | SigSet::from(SIGINT);
            let original_action =
                sigaction_t { sa_mask: org_mask.into(), ..sigaction_t::default() };
    ...
    async fn test_suspend() {
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            let (tx, rx) = m6_starnix_std::sync::mpsc::sync_channel::<()>(0);
            ...
            let closure = move |locked: &mut Locked<Unlocked>, current_task: &CurrentTask| {
                ...
                while !suspended {
                    ...
                    m6_starnix_std::thread::sleep(m6_starnix_std::time::Duration::from_millis(10));
                }
            ...
            assert_eq!(
                sys_rt_sigsuspend(locked, current_task, user_ref, m6_starnix_std::mem::size_of::<SigSet>()),
    ...
    async fn test_waiting_for_child() {
        spawn_kernel_and_run(async |locked, task| {
            ...
            let thread = m6_starnix_std::thread::spawn({
    ...
    async fn test_sigkill() {
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            let address = map_memory(
                ...
                &current_task,
                UserAddress::default(),
                m6_starnix_std::mem::size_of::<i32>() as u64,
            );
    ...
    async fn test_wait4_by_pgid() {
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            m6_starnix_std::mem::drop(child1);
            ...
            m6_starnix_std::mem::drop(child2);
    ...
    async fn test_sigqueue() {
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            let second_current = create_task(locked, current_task.kernel(), "second task");
    ...
    async fn test_signalfd_filters_signals() {
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            let sigchld_mask_addr =
                UserRef::<SigSet>::new((memory_for_masks + m6_starnix_std::mem::size_of::<SigSet>()).unwrap());
            current_task
