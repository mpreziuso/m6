...
use crate::mm_ref::{DumpPolicy, MemoryAccessor, MemoryAccessorExt, PAGE_SIZE};
use crate::ptrace::{
...
#[cfg(feature = "fuchsia")]
use crate::task::SyslogAccess;
use crate::task::{
    CurrentTask, ExitStatus, NormalPriority, SchedulingPolicy, SeccompAction, SeccompStateValue,
    Task, ThreadGroup, max_priority_for_sched_policy, min_priority_for_sched_policy,
};
use crate::vfs::{
...
pub fn sys_clone3(
    ...
    const_assert!(m6_starnix_std::mem::size_of::<clone_args>() == CLONE_ARGS_SIZE_VER2 as usize);
...
fn get_default_cpu_set() -> CpuSet {
    ...
    let mut index = 0;
    while cpus_count > 0 {
        let count = m6_starnix_std::cmp::min(cpus_count, 8);
...
pub fn sys_sched_getaffinity(
    ...
    let mask_size = m6_starnix_std::cmp::min(cpusetsize as usize, CPU_SET_SIZE);
...
pub fn sys_sched_setaffinity(
    ...
    for (l1, l2) in m6_starnix_std::iter::zip(max_mask.bits, mask.bits) {
...
// The syslog manager lives in the gated `syslog`/`Kernel` field; minimal core
// reports ENOSYS for syslog(2) until it is wired.
...
mod tests {
    use super::*;
    use crate::mm_ref::syscalls::sys_munmap;
    ...
    async fn test_get_affinity_size() {
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            assert_eq!(
                ...
                Ok(m6_starnix_std::mem::size_of::<CpuSet>())
            );
            assert_eq!(
    ...
    async fn test_sched_get_priority_min_max() {
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            let min_bad_policy_error =
                sys_sched_get_priority_min(locked, &current_task, u32::MAX).unwrap_err();
            assert_eq!(min_bad_policy_error, errno!(EINVAL));

            let max_bad_policy_error =
                sys_sched_get_priority_max(locked, &current_task, u32::MAX).unwrap_err();
        ...
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            assert_eq!(
                read_c_string_vector(
                    ...
                    argv_addr,
                    100,
                    m6_starnix_std::str::from_utf8(arg).unwrap().trim_matches('\0').len()
                ),
                error!(E2BIG)
            );
        })
