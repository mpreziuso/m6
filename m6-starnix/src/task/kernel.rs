...
use crate::mm_ref::{FutexTable, MappingSummary, MlockPinFlavor, SharedFutexKey};
...
use crate::task::{
    ...
    KernelThreads, PidTable, ThreadGroup, UtsNamespace, UtsNamespaceHandle,
};
#[cfg(feature = "fuchsia")]
use crate::task::{IpTables, KernelCgroups, KernelStats, SchedulerManager, Syslog};
#[cfg(feature = "fuchsia")]
...
use crate::vfs::socket::{
    ...
    NetlinkToClientSender, SocketTokensStore,
};
...
use expando::Expando;
// REMOVED(fidl) use fidl::endpoints::{
//     ClientEnd, ControlHandle, DiscoverableProtocolMarker, ProtocolMarker, create_endpoints,
// };
// REMOVED(fidl) use fidl_fuchsia_component_runner::{ComponentControllerControlHandle, ComponentStopInfo};
// REMOVED(fidl) use fidl_fuchsia_feedback::CrashReporterProxy;
// REMOVED(fidl) use fidl_fuchsia_time_external::AdjustSynchronousProxy;
// REMOVED(fuchsia_inspect) use fuchsia_inspect::ArrayProperty;
#[cfg(feature = "fuchsia")]
...
use m6_starnix_std::sync::OnceLock as OnceCell;
use starnix_lifecycle::{AtomicU32Counter, AtomicU64Counter};
#[cfg(feature = "fuchsia")]
use starnix_logging::log_error;
use starnix_logging::{SyscallLogFilter, log_debug, log_info, log_warn};
use starnix_sync::{
...
use starnix_uapi::VMADDR_CID_HOST;
#[cfg(feature = "fuchsia")]
...
#[cfg(feature = "fuchsia")]
use m6_starnix_std::path::PathBuf;
use m6_starnix_std::sync::atomic::{AtomicBool, AtomicU8, AtomicU16, Ordering};
use m6_starnix_std::sync::{Arc, OnceLock, Weak};
...
impl Kernel {
    /// Minimal-core constructor for M6 first-light. Constructs only the fields the
    /// hello-world + FAT32 path needs; the Fuchsia-runtime subsystems (sockets,
    /// netlink, cgroups, scheduler, inspect, memory attribution, hrtimers, crash
    /// reporting, eBPF) are gated out behind `feature = "fuchsia"`.
    ...
    async fn run_shutdown(&self) {
        ...
        self.expando.remove::<crate::mm_ref::InfoCacheShadowProcess>();
        self.expando.remove::<crate::mm_ref::MlockShadowProcess>();
        ...
        // Normally a Rust program exits its process by calling `m6_starnix_std::process::exit()` which goes
...
impl m6_starnix_std::fmt::Debug for Kernel {
