...
use crate::task::Task;
// RoleOverrides + the FIDL RoleManager are gated with `SchedulerManager`; the
// pure scheduler types below (SchedulerState/SchedulingPolicy/priorities) are
// always available because they are core `Task` state.
#[cfg(feature = "fuchsia")]
use crate::task::RoleOverrides;
// REMOVED(fidl) use fidl::HandleBased;
// REMOVED(fidl) use fidl_fuchsia_scheduler::{
...
impl m6_starnix_std::default::Default for NormalPriority {
    fn default() -> Self {
    ...
    pub fn fork(self) -> Self {
        if self.reset_on_fork {
            let (policy, normal_priority, realtime_priority) = if self.is_realtime() {
                ...
                (
                    self.policy,
                    m6_starnix_std::cmp::min(self.normal_priority, NormalPriority::default()),
                    RealtimePriority::NON_REAL_TIME,
                )
            };
            Self {
...
impl m6_starnix_std::default::Default for SchedulerState {
    fn default() -> Self {
        Self {
