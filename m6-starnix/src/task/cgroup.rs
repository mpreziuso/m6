...
use m6_starnix_std::collections::{BTreeMap, HashMap, HashSet, btree_map, hash_map};
...
impl m6_starnix_std::fmt::Display for FreezerState {
...
impl CgroupState {
    ...
    fn get_effective_freezer_state(&self) -> FreezerState {
        m6_starnix_std::cmp::max(self.self_freezer_state, self.inherited_freezer_state)
    }
    ...
    fn add_process(
        ...
        match pid_table.entry(thread_group.into()) {
            hash_map::Entry::Occupied(mut entry) => {
                ...
                // it already exists.
                if m6_starnix_std::ptr::eq(self, entry.get().as_ptr()) {
                    return Ok(());
                }
