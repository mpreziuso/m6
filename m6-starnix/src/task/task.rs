...
use zx::HandleBased;
use crate::mm_ref::{MemoryAccessor, MemoryAccessorExt, MemoryManager, TaskMemoryAccessor};
...
impl TaskMutableState {
    ...
    pub fn take_captured_state(&mut self) -> Option<Box<CapturedThreadState>> {
        if self.captured_thread_state.is_some() {
            let mut state = None;
            m6_starnix_std::mem::swap(&mut state, &mut self.captured_thread_state);
            return state;
        }
        None
    }
...
impl Task {
    ...
    fn update_scheduler_state_then_role(
        ...
        // M6 minimal core: the kernel scheduler-role manager is a gated Fuchsia
        // subsystem; the scheduler-state is still tracked above, it just isn't
        // pushed to a role manager here.
        #[cfg(feature = "fuchsia")]
        self.thread_group().kernel.scheduler.set_thread_role(self, new_scheduler_state)?;
        #[cfg(not(feature = "fuchsia"))]
        let _ = new_scheduler_state;
        Ok(())
    }
    ...
    pub fn read_argv(&self, max_len: usize) -> Result<Vec<FsString>, Errno> {
        ...
        let len_to_read = m6_starnix_std::cmp::min(argv_end - argv_start, max_len);
    ...
    pub fn read_env(&self, max_len: usize) -> Result<Vec<FsString>, Errno> {
        ...
        let len_to_read = m6_starnix_std::cmp::min(env_end - env_start, max_len);
    ...
    // pid→koid mapping is only used by the (gated) tracing subsystem.
    #[cfg(not(feature = "fuchsia"))]
    pub fn record_pid_koid_mapping(&self) {}
}
