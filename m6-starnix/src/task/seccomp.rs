...
use crate::mm_ref::MemoryAccessorExt;
...
use bstr::ByteSlice;
// seccomp filters compile cBPF to eBPF and run them on the ebpf VM. That VM
// (the `ebpf`/`ebpf_api` crates) is gated; the minimal core compiles filters to
// ENOSYS and treats any (never-installed) filter as SECCOMP_RET_ALLOW. Real
// seccomp filtering returns with an eBPF VM port (M5).
#[cfg(feature = "fuchsia")]
use ebpf::{
...
use m6_starnix_std::sync::atomic::{AtomicU8, Ordering};
...
pub struct SeccompFilter {
    /// The BPF program associated with this filter (gated eBPF VM).
    #[cfg(feature = "fuchsia")]
...
impl SeccompFilter {
    ...
    pub fn from_cbpf(
        _code: &Vec<sock_filter>,
        _maybe_unique_id: u64,
        _should_log: bool,
    ) -> Result<Self, Errno> {
        // eBPF VM gated: seccomp filter installation is unsupported in the
        // minimal core.
        error!(ENOSYS)
    }
    ...
    pub fn from_cbpf(
        ...
        for insn in code {
            ...
            if (bpf_class(insn) == BPF_LD || bpf_class(insn) == BPF_ST)
                ...
                && (insn.k & 0x3 != 0 || insn.k as usize >= m6_starnix_std::mem::size_of::<seccomp_data>())
            {
                return error!(EINVAL);
            }
    ...
    // No eBPF VM in the minimal core; any (never-installed) filter allows.
    #[cfg(not(feature = "fuchsia"))]
    pub fn run(&self, _data: &seccomp_data) -> u32 {
        // SECCOMP_RET_ALLOW
        0x7fff_0000
    }
}
...
impl SeccompFilterContainer {
    ...
    pub fn run_all(&self, current_task: &CurrentTask, syscall: &Syscall) -> SeccompFilterResult {
        ...
        for filter in self.filters.iter().rev() {
            ...
            if SeccompAction::has_prio(&action, &r.action) == m6_starnix_std::cmp::Ordering::Less {
...
impl SeccompState {
    ...
    fn log_action(task: &CurrentTask, syscall: &Syscall) {
        ...
        // M6 minimal core: no audit-log subsystem yet (gated). Consume the
        // gathered fields so the diagnostic plumbing stays in place.
        let _ = (uid, gid, arch);
        #[cfg(feature = "fuchsia")]
...
impl SeccompAction {
    ...
    pub fn from_u32(action: u32) -> Option<SeccompAction> {
        match action & !SECCOMP_RET_DATA {
            ...
            linux_uapi::SECCOMP_RET_ERRNO => {
                ...
                action = m6_starnix_std::cmp::min(action & 0xffff, 0xfff);
    ...
    pub fn has_prio(a: &SeccompAction, b: &SeccompAction) -> m6_starnix_std::cmp::Ordering {
    ...
    fn ioctl(
        ...
        match request {
            SECCOMP_IOCTL_NOTIF_RECV => {
                if let Ok(notif) =
                    current_task.read_memory_to_vec(user_addr, m6_starnix_std::mem::size_of::<seccomp_notif>())
                {
...
mod test {
    ...
    async fn test_actions_logged_accepts_legal_string() {
        spawn_kernel_and_run(async |_, current_task| {
            ...
            let action_string = m6_starnix_std::string::String::from_utf8(actions.clone()).unwrap();
            ...
            assert!(
                ...
                m6_starnix_std::string::String::from_utf8(actions.clone()).unwrap(),
