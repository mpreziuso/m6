...
use m6_starnix_std::sync::atomic::{AtomicBool, Ordering};
use m6_starnix_std::sync::{Arc, Weak};
use zx::{AsHandleRef, Koid, Status, Task as _};
...
impl m6_starnix_std::ops::Deref for ThreadGroupKey {
...
pub struct ThreadGroupMutableState {
    ...
    /// Notifier for name changes (memory-attribution; gated out of minimal core).
    #[cfg(feature = "fuchsia")]
    pub notifier: Option<m6_starnix_std::sync::mpsc::Sender<MemoryAttributionLifecycleEvent>>,
}
...
impl Ord for ZombieProcess {
    fn cmp(&self, other: &Self) -> m6_starnix_std::cmp::Ordering {
...
impl ThreadGroup {
        ...
        L: LockBefore<ProcessGroupState>,
    {
        ...
        if state.tasks.is_empty() {
            ...
            // more in lock_cgroup2_pid_table comments. (cgroups gated out of minimal core.)
            #[cfg(feature = "fuchsia")]
...

            {
                ...
                if let Some(reaper) = reaper {
                    let reaper = reaper.upgrade();
                    {
                        ...
                        let mut state = self.write();
                        for (_pid, weak_child) in m6_starnix_std::mem::take(&mut state.children) {
        ...
        L: LockBefore<ProcessGroupState>,
    {
        ...
        // M6: `itertools::unique` needs std (HashMap); dedup the process-group
        // Arcs by pointer identity instead.
        let mut process_groups: Vec<_> = Vec::new();
        for tg in thread_groups.iter() {
            let pg = Arc::clone(&tg.read().process_group);
            if !process_groups.iter().any(|existing| Arc::ptr_eq(existing, &pg)) {
    ...
    ///
    /// Gated: graceful shutdown uses the Fuchsia async Timer + `select_biased!`
    /// (futures-macro) and is only driven by the (gated) kernel `run_shutdown`.
    #[cfg(feature = "fuchsia")]
...
mod test {
    ...
    async fn test_getppid_after_self_and_parent_exit() {
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            m6_starnix_std::mem::drop(task1);
            m6_starnix_std::mem::drop(tg1);
