...
pub struct PidTable {
    ...
    /// Used to notify thread group changes (memory-attribution; gated out of
    /// the minimal core).
    #[cfg(feature = "fuchsia")]
    thread_group_notifier: RcuOptionCell<m6_starnix_std::sync::mpsc::Sender<MemoryAttributionLifecycleEvent>>,
}
...
impl PidTable {
    ...
    pub fn set_thread_group_notifier(
        &self,
        notifier: m6_starnix_std::sync::mpsc::Sender<MemoryAttributionLifecycleEvent>,
    ) {
    ...
    pub fn add_task(&mut self, task: &TempRef<'_, Task>) {
        ...
        if task.is_leader() {
            ...
            // Notify thread group changes (memory-attribution; gated).
            #[cfg(feature = "fuchsia")]
            {
