...
impl m6_starnix_std::fmt::Debug for PortWaiter {
...
impl Drop for Waiter {
    fn drop(&mut self) {
        ...
        let wait_queues = m6_starnix_std::mem::take(&mut *self.inner.wait_queues.lock()).into_values();
...
pub struct TypedWaitQueue<T: Into<u64>> {
    ...
    value_type: m6_starnix_std::marker::PhantomData<T>,
}
...
mod tests {
    ...
    async fn waiter_kind_abort_handle() {
        spawn_kernel_and_run_sync(|_locked, current_task| {
            ...
            assert_matches!(executor.run_until_stalled(&mut fut), m6_starnix_std::task::Poll::Pending);
