...
// REMOVED(fidl) use fidl_fuchsia_diagnostics as fdiagnostics;
// REMOVED(fuchsia_component) use fuchsia_component::client::connect_to_protocol_sync;
// REMOVED(fuchsia_inspect) use fuchsia_inspect::Inspector;
...
use m6_starnix_std::io::{self, Write};
use m6_starnix_std::sync::atomic::Ordering;
use m6_starnix_std::sync::{Arc, OnceLock, mpsc};
...
impl Into<Vec<u8>> for ResultBuffer {
    fn into(self) -> Vec<u8> {
        ...
        let size = m6_starnix_std::cmp::min(result.len(), m6_starnix_std::cmp::min(self.max_size, self.current_size));
        if result.len() != size {
...
pub(crate) fn extract_level(msg: &[u8]) -> Option<(KmsgLevel, &[u8])> {
    ...
    m6_starnix_std::str::from_utf8(&msg[1..end])
        .ok()
