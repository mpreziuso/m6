...
use m6_starnix_std::ffi::{CStr, CString};
use m6_starnix_std::mem::size_of;
use m6_starnix_std::ops::Deref as _;
...
impl elf_load::Mapper for Mapper {
    fn map(
        ...
        // `length` is the full segment (memsz) including the .bss tail, which
        // extends past the file content. Give each segment its own VMO sized to
        // `length`, copy in only the `file_length` file-backed bytes, and leave
        // the tail zero. Mapping the shared file VMO directly would leave .bss
        // beyond its `size` (kernel reads/writes fault EFAULT) and writes would
        // corrupt the file; copying past `file_length` would pull in following
        // file bytes (.shstrtab etc.) and corrupt .bss.
        let seg_vmo = zx::Vmo::create(length as u64)?;
        let file_size = vmo.get_size()?;
        let avail = file_size.saturating_sub(vmo_offset).min(file_length as u64) as usize;
        if avail > 0 {
            let mut buf = vec![0u8; avail];
            vmo.read(&mut buf, vmo_offset)?;
            seg_vmo.write(&buf, 0)?;
        }
        let memory = Arc::new(MemoryObject::from(seg_vmo));
        self.mm
            .map_memory(
                ...
                memory,
                // The segment VMO holds the file-backed bytes starting at
                // offset 0 (see the copy above), so the mapping offset is 0.
                0,
                length,
...
pub fn load_executable(
    ...
    //
    // M6 bring-up: upstream pads by 0xf0000 (~960 KiB) because Fuchsia's `MAX_STACK_SIZE`
    // is 512 MiB and the stack grows down on demand. M6's zx-shim `Vmar::map` is *eager*
    // (commits a frame per page, no demand paging), so the whole pad would be committed up
    // front — that both exceeds the capped `MAX_STACK_SIZE` (256 KiB) tripping `map_stack`'s
    // assert, and burns mem_context cnode slots that collide with the m6-std heap. Until
    // demand paging lands, use a small fixed pad: the eagerly-mapped region is the entire
    // usable stack, which is ample for first-light static binaries.
    const M6_STACK_PAD: usize = 0x1e000; // ~120 KiB
