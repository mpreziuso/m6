...
// REMOVED(fidl) use fidl_fuchsia_storage_block::BlockMarker;
...
use m6_starnix_std::collections::btree_map::BTreeMap;
use m6_starnix_std::sync::Arc;
use m6_starnix_std::sync::atomic::{AtomicU32, Ordering};
...
impl SyncBlockClient {
    fn new(kthreads: &KernelThreads, block: ClientEnd<BlockMarker>) -> Result<Arc<Self>, Errno> {
        let (init_tx, init_rx) = m6_starnix_std::sync::mpsc::channel();
...
impl FileOps for RemoteBlockDeviceFile {
    ...
    fn read(
        ...
        while data.available() > 0 && offset < size {
            let chunk_len = m6_starnix_std::cmp::min(data.available(), MAX_CHUNK_SIZE);
            let chunk_len = m6_starnix_std::cmp::min(chunk_len, size - offset);
            if chunk_len == 0 {
                break;
            }
            ...
            let aligned_end_offset = m6_starnix_std::cmp::min(
        ...
        while data.available() > 0 && offset < size {
            ...
            let write_slice_uninit = unsafe {
                m6_starnix_std::slice::from_raw_parts_mut(
                    write_slice.as_mut_ptr() as *mut m6_starnix_std::mem::MaybeUninit<u8>,
                    write_slice.len(),
                )
            };
