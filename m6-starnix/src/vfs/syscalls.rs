...
use crate::mm_ref::{IOVecPtr, MemoryAccessor, MemoryAccessorExt, PAGE_SIZE};
use crate::security;
...
pub fn sys_openat2(
    ...
    const EXPECTED_SIZE: usize = m6_starnix_std::mem::size_of::<uapi::open_how>();
    ...
    while pos < size {
        let length = m6_starnix_std::cmp::min(size - pos, *PAGE_SIZE as usize);
        let extra_bytes =
...
pub fn sys_readlinkat(
    ...
    let length = m6_starnix_std::cmp::min(buffer_size, target.len());
...
// timerfd is backed by the gated HrTimer/timerfd subsystem; minimal core reports
// ENOSYS until the clock-object model lands.
...
pub fn sys_timerfd_gettime(
    ...
    _current_task: &CurrentTask,
    _fd: FdNumber,
    _user_current_value: ITimerSpecPtr,
) -> Result<(), Errno> {
    error!(ENOSYS)
}
...
pub fn sys_timerfd_settime(
    ...
    _fd: FdNumber,
    _flags: u32,
    _user_new_value: ITimerSpecPtr,
    _user_old_value: ITimerSpecPtr,
) -> Result<(), Errno> {
    error!(ENOSYS)
}
...
fn select(
    ...
    fn sizeof<T>(_: &T) -> usize {
        BITS_PER_BYTE * m6_starnix_std::mem::size_of::<T>()
    }
    ...
    if nfds as usize > BITS_PER_BYTE * m6_starnix_std::mem::size_of::<__kernel_fd_set>() {
        return error!(EINVAL);
    }
    ...
    let mask = if !sigmask_addr.is_null() {
        ...
        let mask = if sigmask.ss.is_null() {
            ...
            if sigmask.ss_len < m6_starnix_std::mem::size_of::<sigset_t>() {
                return error!(EINVAL);
            }
...
pub fn sys_pselect6(
            ...
            .contains(PersonalityFlags::STICKY_TIMEOUTS)
    {
        ...
        let remaining = m6_starnix_std::cmp::max(deadline - now, zx::MonotonicDuration::from_seconds(0));
...
fn do_epoll_pwait(
    ...
    current_task
        .mm()?
        .check_plausible(events.addr(), max_events * m6_starnix_std::mem::size_of::<EpollEvent>())?;
...
pub fn sys_io_destroy(
    ...
    m6_starnix_std::mem::drop(aio_context);
    Ok(())
}
...
mod arch32 {
    ...
    pub fn sys_arch32_fstatfs64(
        ...
        if (user_buf_len as usize) < m6_starnix_std::mem::size_of::<uapi::arch32::statfs64>() {
            return error!(EINVAL);
        }
