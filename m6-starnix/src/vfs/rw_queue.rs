...
#[cfg(any(test, debug_assertions))]
mod tracer {
    // Upstream uses `tracing_mutex` here to detect lock-ordering cycles in debug
    // builds. M6 has no such crate yet (and it is not no_std), so `MutexTracer`
    // is a no-op. Lock-cycle detection can be reintroduced when an no_std
    // equivalent exists; the call sites (`lock_shared`/`lock_exclusive`/
    // `unlock_*`) stay so re-enabling it is a one-type change.
    #[derive(Debug, Default)]
    pub struct MutexTracer;
...
mod test {
    ...
    use m6_starnix_std::future::Future;
    use m6_starnix_std::pin::Pin;
    use m6_starnix_std::sync::Barrier;
    use m6_starnix_std::sync::atomic::{AtomicUsize, Ordering};
