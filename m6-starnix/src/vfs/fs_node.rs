...
use m6_starnix_std::sync::OnceBool;
    ...
    fn ensure_fifo(&self, current_task: &CurrentTask) -> &PipeHandle {
        self.fifo.get_or_init(|| {
            ...
            if !security::is_task_capable_noaudit(current_task, CAP_SYS_RESOURCE) {
                ...
                default_pipe_capacity = m6_starnix_std::cmp::min(default_pipe_capacity, max_size);
            }
    ...
    pub fn flock(
        ...
        loop {
            ...
            if operation.is_unlock() {
                flock_info.retain(|fh| !m6_starnix_std::ptr::eq(fh, self));
                return Ok(());
            }
            ...
            let fd_has_lock = flock_info
                ...
                .find_map(|w| {
                    w.upgrade().and_then(|fh| {
                        if m6_starnix_std::ptr::eq(&fh as &FileObject, self) { Some(()) } else { None }
                    })
                })
                .is_some();
            ...
            m6_starnix_std::mem::drop(flock_info);
...
impl FsNode {
        ...
        L: LockEqualOrBefore<FileOpsCore>,
    {
        self.check_access(
            ...
            Access::EXEC,
            CheckAccessReason::InternalPermissionChecks,
            &[Auditable::Name(name), m6_starnix_std::panic::Location::caller().into()],
        )?;
        let locked = locked.cast_locked::<FileOpsCore>();
        ...
        L: LockEqualOrBefore<FileOpsCore>,
    {
        self.check_access(
            ...
            Access::WRITE,
            CheckAccessReason::InternalPermissionChecks,
            security::Auditable::Location(m6_starnix_std::panic::Location::caller()),
        )?;
        self.update_metadata_for_child(current_task, &mut mode, &mut owner);
...
impl m6_starnix_std::fmt::Debug for FsNode {
