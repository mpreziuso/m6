...
use crate::mm_ref::{DesiredAddress, MappingName, MappingOptions, MemoryAccessorExt, ProtectionFlags};
...
// REMOVED(fidl) use fidl::endpoints::ProtocolMarker as _;
...
pub trait FileOps: Send + Sync + AsAny + 'static {
    ...
    /// and then maps it with [`crate::mm_ref::MemoryManager::map`].
    ...
    // Without the (gated) zxio file server there is no Zircon handle to hand out,
    // so the descriptor behaves as a fd to `/dev/null` (per the doc above).
    ...
    match canonicalize_ioctl_request(current_task, request) {
        ...
        FS_IOC_SET_ENCRYPTION_POLICY => {
            ...
            if let Some(wrapping_key_id) = &attributes.wrapping_key_id {
                ...
                m6_starnix_std::mem::drop(attributes);
                file.node().update_attributes(locked, current_task, |info| {
...
impl m6_starnix_std::ops::Deref for FileObject {
