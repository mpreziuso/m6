...
use m6_starnix_std::collections::BTreeSet;
...
impl m6_starnix_std::ops::Add<usize> for RecordLength {
    type Output = Self;
...
impl m6_starnix_std::ops::Sub<usize> for RecordLength {
...
impl m6_starnix_std::cmp::PartialEq<usize> for RecordLength {
...
impl m6_starnix_std::cmp::PartialOrd<usize> for RecordLength {
    fn partial_cmp(&self, other: &usize) -> Option<m6_starnix_std::cmp::Ordering> {
        match self {
            Self::Value(e) => e.partial_cmp(other),
            Self::Infinite => Some(m6_starnix_std::cmp::Ordering::Greater),
        }
    }
}
...
impl Ord for RecordLength {
    fn cmp(&self, other: &Self) -> m6_starnix_std::cmp::Ordering {
        if self == other {
            m6_starnix_std::cmp::Ordering::Equal
        } else {
            match self {
                Self::Value(e1) => match other {
                    Self::Value(e2) => e1.cmp(e2),
                    Self::Infinite => m6_starnix_std::cmp::Ordering::Less,
                },
                Self::Infinite => m6_starnix_std::cmp::Ordering::Greater,
            }
        }
    }
}
...
impl RecordRange {
    ...
    fn intersects(&self, other: &RecordRange) -> bool {
        let r1 = m6_starnix_std::cmp::min(self, other);
        let r2 = m6_starnix_std::cmp::max(self, other);
        r1.end() > r2.start
    }
}

impl m6_starnix_std::ops::Sub<RecordRange> for RecordRange {
    ...
    fn sub(self, other: RecordRange) -> Vec<RecordRange> {
        ...
        if self.start < other.start {
            let length = m6_starnix_std::cmp::min(RecordLength::Value(other.start - self.start), self.length);
...
impl m6_starnix_std::ops::Add<RecordRange> for RecordRange {
    type Output = Vec<Self>;
    ...
    fn add(self, other: RecordRange) -> Vec<RecordRange> {
        ...
        if r1_end < r2.start {
            ...
            let end = m6_starnix_std::cmp::max(r1_end, r2.end());
            vec![RecordRange {
                start: r1.start,
