...
use m6_starnix_std::ops::{Deref, DerefMut};
...
fn slice_to_maybe_uninit(buffer: &[u8]) -> &[MaybeUninit<u8>] {
    ...
    unsafe { m6_starnix_std::slice::from_raw_parts(buffer.as_ptr() as *const MaybeUninit<u8>, buffer.len()) }
}
...
pub struct IovecsRef<'a, I: Sized> {
    ...
    _marker: m6_starnix_std::marker::PhantomData<&'a I>,
}
...
pub trait Buffer: m6_starnix_std::fmt::Debug {
    ...
    /// for any non-zero sized access (see Rust's m6_starnix_std::ptr documentation).
...
/// sized access (see Rust's m6_starnix_std::ptr documentation).
...
pub trait OutputBuffer: Buffer {
    ...
    fn write(&mut self, buffer: &[u8]) -> Result<usize, Errno> {
        ...
        self.write_each(&mut move |data| {
            let size = m6_starnix_std::cmp::min(buffer.len(), data.len());
    ...
    fn write_buffer(&mut self, input: &mut dyn InputBuffer) -> Result<usize, Errno> {
        self.write_each(&mut move |data| {
            let size = m6_starnix_std::cmp::min(data.len(), input.available());
...
pub trait InputBuffer: Buffer {
    ...
    fn peek(&mut self, buffer: &mut [MaybeUninit<u8>]) -> Result<usize, Errno> {
        ...
        self.peek_each(&mut move |data| {
            ...
            let size = m6_starnix_std::cmp::min(buffer.len() - index, data.len());
...
impl<'a, M> m6_starnix_std::fmt::Debug for UserBuffersOutputBuffer<'a, M> {
...
impl<'a, M: TaskMemoryAccessor> OutputBuffer for UserBuffersOutputBuffer<'a, M> {
    fn write(&mut self, mut bytes: &[u8]) -> Result<usize, Errno> {
        self.write_each_inner(|buflen| {
            let bytes_len = m6_starnix_std::cmp::min(bytes.len(), buflen);
    ...
    unsafe fn advance(&mut self, mut length: usize) -> Result<(), Errno> {
        ...
        while let Some(mut buffer) = self.buffers.pop() {
            ...
            let advance_by = m6_starnix_std::cmp::min(length, buffer.length);
...
impl<'a, M> m6_starnix_std::fmt::Debug for UserBuffersInputBuffer<'a, M> {
    fn fmt(&self, f: &mut m6_starnix_std::fmt::Formatter<'_>) -> m6_starnix_std::fmt::Result {
...
impl<'a, M: TaskMemoryAccessor> InputBuffer for UserBuffersInputBuffer<'a, M> {
    fn peek(&mut self, uninit_bytes: &mut [MaybeUninit<u8>]) -> Result<usize, Errno> {
        self.peek_each_inner(|buffer, read_so_far| {
            ...
            let read_count = m6_starnix_std::cmp::min(buffer.length, read_to.len());
...
impl VecInputBuffer {
    ...
    pub fn read_object<T: FromBytes>(&mut self) -> Result<T, Errno> {
        let size = m6_starnix_std::mem::size_of::<T>();
...
mod tests {
    use super::*;
    use crate::mm_ref::{MemoryAccessor as _, PAGE_SIZE};
    use crate::testing::*;
