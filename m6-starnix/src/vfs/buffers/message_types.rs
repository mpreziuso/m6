...
use crate::mm_ref::VmsplicePayload;
use crate::security;
...
impl UnixControlData {
    ...
    pub fn new(current_task: &CurrentTask, message: ControlMsg) -> Result<Self, Errno> {
        match message.r#type {
            SCM_RIGHTS => {
                ...
                let bytes_per_file_descriptor = m6_starnix_std::mem::size_of::<FdNumber>();
    ...
    pub fn total_size(&self, current_task: &CurrentTask) -> usize {
        match self {
            UnixControlData::Rights(files) => files.len() * m6_starnix_std::mem::size_of::<FdNumber>(),
    ...
    pub fn minimum_size(&self) -> usize {
        match self {
            UnixControlData::Rights(_files) => m6_starnix_std::mem::size_of::<FdNumber>(),
...
impl MessageData for Vec<u8> {
    ...
    fn clone_at_most(&self, limit: usize) -> Self {
        self[0..m6_starnix_std::cmp::min(self.len(), limit)].to_vec()
    }
