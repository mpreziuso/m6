...
// Upstream hashes fs-verity descriptors with the `mundane` crate. M6 has no
// hash crate wired yet; `fsverity_measurement` reports unsupported until one is
// (fs-verity is not on the first-light path).
...
fn fsverity_descriptor_from_enable_arg(
    ...
    if src.block_size.count_ones() != 1
        || src.block_size < 1024
        || src.block_size > m6_starnix_std::cmp::min(4096, filesystem_block_size)
    {
        return error!(EINVAL);
    }
...
fn fsverity_measurement(descriptor: &fsverity_descriptor) -> Result<Box<[u8]>, Errno> {
    match descriptor.hash_algorithm.into() {
        // SHA-256/512 hashing of the descriptor is not yet available on M6 (no
        // hash crate wired). fs-verity is off the first-light path, so report
        // unsupported rather than pulling in a host-only crate.
        FS_VERITY_HASH_ALG_SHA256 | FS_VERITY_HASH_ALG_SHA512 => error!(ENOTSUP),
        _ => error!(EINVAL),
    }
}
