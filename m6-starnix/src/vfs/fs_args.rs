...
impl MountParams {
    ...
    pub fn get(&self, key: &[u8]) -> Option<&FlyByteStr> {
        self.options.get(&FlyByteStr::from(key))
    }

    pub fn get_as<T: m6_starnix_std::str::FromStr>(&self, key: &[u8]) -> Result<Option<T>, Errno>
    where
        <T as m6_starnix_std::str::FromStr>::Err: m6_starnix_std::fmt::Debug,
    {
        self.get(key).map(|v| parse::<T>(v.as_ref())).transpose()
    }

    pub fn get_with<T, E: m6_starnix_std::fmt::Debug>(
        &self,
        key: &[u8],
    ...
    pub fn remove(&mut self, key: &[u8]) -> Option<FlyByteStr> {
        self.options.remove(&FlyByteStr::from(key))
    }
...
pub fn parse<F: m6_starnix_std::str::FromStr>(data: &FsStr) -> Result<F, Errno>
where
    <F as m6_starnix_std::str::FromStr>::Err: m6_starnix_std::fmt::Debug,
{
...
///
/// This relies on str::parse so expects `data` to be utf8.
pub fn parse_with<F, E: m6_starnix_std::fmt::Debug>(
    data: &FsStr,
    parser: impl FnOnce(&str) -> Result<F, E>,
) -> Result<F, Errno> {
    parser(m6_starnix_std::str::from_utf8(data.as_ref()).map_err(|e| errno!(EINVAL, e))?.trim())
