...
use m6_starnix_std::borrow::Borrow;
use m6_starnix_std::collections::HashSet;
use m6_starnix_std::fmt;
use m6_starnix_std::hash::{Hash, Hasher};
...
impl m6_starnix_std::cmp::PartialEq for MountInfo {
    fn eq(&self, other: &Self) -> bool {
        self.handle.as_ref().map(Arc::as_ptr) == other.handle.as_ref().map(Arc::as_ptr)
    }
}

impl m6_starnix_std::cmp::Eq for MountInfo {}
