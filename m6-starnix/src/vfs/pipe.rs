...
impl Pipe {
    ...
    pub fn open(
        ...
        loop {
            ...
            m6_starnix_std::mem::drop(pipe_locked);
    ...
    fn set_capacity(
        ...
        if requested_capacity
            > current_task
                ...
                .system_limits
                .pipe_max_size
                .load(m6_starnix_std::sync::atomic::Ordering::Relaxed)
        {
    ...
    pub fn splice(from: &mut Pipe, to: &mut Pipe, len: usize) -> Result<usize, Errno> {
        ...
        loop {
            let limit = m6_starnix_std::cmp::min(len - bytes_transferred, to.messages.available_capacity());
            if limit == 0 {
...
impl<'a> Buffer for SpliceInputBuffer<'a> {
    ...
    fn peek_each_segment(
        ...
        for message in self.pipe.messages.messages() {
            let to_read = m6_starnix_std::cmp::min(available, message.len());
            callback(&UserBuffer {
...
impl PipeFileObject {
        ...
        L: LockEqualOrBefore<FileOpsCore>,
    {
        ...
        let len = m6_starnix_std::cmp::min(len, pipe.messages.available_capacity());
        ...
        L: LockEqualOrBefore<FileOpsCore>,
    {
        ...
        let len = m6_starnix_std::cmp::min(len, pipe.messages.len());
        ...
        L: LockEqualOrBefore<FileOpsCore>,
    {
        ...
        let was_empty = pipe.is_empty();
        let mut remaining = m6_starnix_std::cmp::min(available, pipe.messages.available_capacity());
        ...
        L: LockEqualOrBefore<FileOpsCore>,
    {
        ...
        let len = m6_starnix_std::cmp::min(data.available(), pipe.messages.len());
        let mut buffer = SpliceInputBuffer { pipe: &mut pipe, len, available: len };
