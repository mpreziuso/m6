...
use anyhow::Context;
// REMOVED(fidl) use fidl::endpoints::{SynchronousProxy, create_sync_proxy};
// REMOVED(fidl) use fidl_fuchsia_hardware_qualcomm_router as fqrtr;
...
fn connect_to_connector() -> Result<fqrtr::QrtrConnectorSynchronousProxy, anyhow::Error> {
    let mut dir = m6_starnix_std::fs::read_dir(QRTR_CLIENT_SERVICE_DIRECTORY)
