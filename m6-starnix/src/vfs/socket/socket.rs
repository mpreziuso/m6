...
impl SockOptValue {
    ...
    pub fn read_bytes(
        ...
        match self {
            Self::Value(buffer) => {
                let bytes = m6_starnix_std::cmp::min(max_bytes, buffer.len());
                Ok(buffer[..bytes].to_owned())
            }
            Self::User(user_buffer) => {
                let bytes = m6_starnix_std::cmp::min(max_bytes, user_buffer.length);
                current_task
...
impl Socket {
        ...
        L: LockEqualOrBefore<FileOpsCore>,
    {
        ...
        let backlog = m6_starnix_std::cmp::min(backlog, max_connections);
...
mod tests {
    ...
    async fn test_dgram_socket() {
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            let passcred: u32 = 1;
            let opt_size = m6_starnix_std::mem::size_of::<u32>();
            let user_address =
