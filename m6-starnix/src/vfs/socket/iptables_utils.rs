...
mod tests {
    ...
    fn ipv4_subnet_addr() -> in_addr {
        let bytes = "192.0.2.0".parse::<m6_starnix_std::net::Ipv4Addr>().unwrap().octets();
        in_addr { s_addr: u32::from_be_bytes(bytes).to_be() }
    }

    fn ipv4_subnet_mask() -> in_addr {
        let bytes = "255.255.255.0".parse::<m6_starnix_std::net::Ipv4Addr>().unwrap().octets();
        in_addr { s_addr: u32::from_be_bytes(bytes).to_be() }
    }
    ...
    fn ipv6_subnet_addr() -> in6_addr {
        let bytes = "2001:db8::".parse::<m6_starnix_std::net::Ipv6Addr>().unwrap().octets();
        in6_addr { in6_u: in6_addr__bindgen_ty_1 { u6_addr8: bytes } }
    }

    fn ipv6_subnet_mask() -> in6_addr {
        let bytes = "ffff:ffff::".parse::<m6_starnix_std::net::Ipv6Addr>().unwrap().octets();
        in6_addr { in6_u: in6_addr__bindgen_ty_1 { u6_addr8: bytes } }
    }
