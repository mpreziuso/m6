...
fn write_socket_address(
    ...
    if length > 0 {
        let actual = m6_starnix_std::cmp::min(length, capacity) as usize;
    ...
    L: LockEqualOrBefore<FileOpsCore>,
{
    ...
    if !msg_name.is_null() {
        ...
        let num_bytes = m6_starnix_std::cmp::min(message_header.name_len as usize, bytes.len());
    ...
    L: LockEqualOrBefore<FileOpsCore>,
{
    ...
    loop {
        ...
        let data_size = m6_starnix_std::cmp::min(cmsg.cmsg_len as usize - header_size, space);
