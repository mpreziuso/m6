...
pub const SA_FAMILY_SIZE: usize = m6_starnix_std::mem::size_of::<uapi::__kernel_sa_family_t>();
pub const SA_STORAGE_SIZE: usize = m6_starnix_std::mem::size_of::<uapi::sockaddr_storage>();
...
impl SocketAddress {
    ...
    pub fn from_bytes(address: Vec<u8>) -> Result<SocketAddress, Errno> {
        ...
        let address = match family {
            ...
            AF_INET => {
                let sockaddr_len = m6_starnix_std::mem::size_of::<sockaddr_in>();
                let addrlen = m6_starnix_std::cmp::min(address.len(), sockaddr_len);
                SocketAddress::Inet(address[..addrlen].to_vec())
            }
            AF_INET6 => {
                let sockaddr_len = m6_starnix_std::mem::size_of::<sockaddr_in6>();
                let addrlen = m6_starnix_std::cmp::min(address.len(), sockaddr_len);
            ...
            AF_PACKET => {
                let sockaddr_len = m6_starnix_std::mem::size_of::<sockaddr_ll>();
                let addrlen = m6_starnix_std::cmp::min(address.len(), sockaddr_len);
                SocketAddress::Packet(address[..addrlen].to_vec())
            }
            AF_QIPCRTR => {
                let sockaddr_len = m6_starnix_std::mem::size_of::<sockaddr_qrtr>();
                let addrlen = m6_starnix_std::cmp::min(address.len(), sockaddr_len);
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        match self {
            ...
            SocketAddress::Unix(name) => match name.first() {
                ...
                Some(_) => {
                    let template = sockaddr_un::default();
                    let path_length = m6_starnix_std::cmp::min(template.sun_path.len() - 1, name.len());
                    [&AF_UNIX.to_ne_bytes(), &name[..path_length], &[b'\0']].concat()
                }
            },
            SocketAddress::Vsock { port, cid } => {
                let mut bytes = vec![0u8; m6_starnix_std::mem::size_of::<sockaddr_vm>()];
