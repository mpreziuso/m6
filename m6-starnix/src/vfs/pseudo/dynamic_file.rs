...
impl<Source: SequenceFileSource> DynamicFileState<Source> {
    ...
    fn read(
        ...
        while self.byte_offset + self.buf.0.len() < offset + read_size {
            let cursor = if let Some(cursor) = m6_starnix_std::mem::take(&mut self.cursor) {
                cursor
            } else {
                break;
            };
            let mut buf = m6_starnix_std::mem::take(&mut self.buf);
            self.cursor =
            ...
            let to_drain = m6_starnix_std::cmp::min(offset - self.byte_offset, self.buf.0.len());
...
impl DynamicFileBuf {
    ...
    pub fn write_fmt(&mut self, args: m6_starnix_std::fmt::Arguments<'_>) -> Result<usize, Errno> {
        let start_size = self.0.len();
        m6_starnix_std::io::Write::write_fmt(&mut self.0, args).map_err(|_| errno!(EINVAL))?;
