...
use crate::mm_ref::ProtectionFlags;
...
// REMOVED(fidl) use fidl::HandleBased;
// REMOVED(fidl) use fidl::endpoints::{ClientEnd, ServerEnd};
// REMOVED(fidl) use fidl_fuchsia_io as fio;
// REMOVED(fuchsia_runtime) use fuchsia_runtime::UtcInstant;
...
use m6_starnix_std::sync::atomic::{AtomicU64, Ordering};
...
struct EitherSender<R>(Either<futures::channel::oneshot::Sender<R>, m6_starnix_std::sync::mpsc::Sender<R>>);
...
impl<R> From<m6_starnix_std::sync::mpsc::Sender<R>> for EitherSender<R> {
    fn from(v: m6_starnix_std::sync::mpsc::Sender<R>) -> Self {
        Self(Either::Right(v))
    }
}
...
async fn handle_file(
    ...
    credentials: Arc<Credentials>,
    file: FileHandle,
    receiver: m6_starnix_std::sync::mpsc::Receiver<Box<dyn Work>>,
) {
...
struct StarnixNodeConnection {
    is_dir: bool,
    credentials: Arc<Credentials>,
    work_sender: m6_starnix_std::sync::mpsc::Sender<Box<dyn Work>>,
...
impl StarnixNodeConnection {
    fn new(
        ...
        let (work_sender, receiver) = m6_starnix_std::sync::mpsc::channel();
    ...
    where
        ...
        Errno: From<E>,
    {
        let (sender, receiver) = m6_starnix_std::sync::mpsc::channel();
        self.work_sender
...
mod tests {
    ...
    use m6_starnix_std::collections::HashSet;
    ...
    async fn access_file_system() {
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            m6_starnix_std::thread::spawn(move || {
