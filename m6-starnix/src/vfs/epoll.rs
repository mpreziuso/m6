...
use m6_starnix_std::collections::hash_map::Entry;
...
impl EpollFileObject {
        ...
        L: LockEqualOrBefore<FileOpsCore>,
    {
        ...
        let Some(wait_object) = state.wait_objects.get_mut(&ReadyItemKey::from(key)) else {
            return error!(ENOENT);
        };
        if let Some(wait_canceler) = wait_object.wait_canceler.take() {
            wait_canceler.cancel();
        ...
        L: LockEqualOrBefore<FileOpsCore>,
    {
        {
            let mut state = self.state.lock();
            let recheck_list = m6_starnix_std::mem::take(&mut state.recheck_list);
        ...
        let mut state = self.state.lock();
        let state = &mut *state;
        // M6: `itertools::unique_by` needs std (HashMap); dedup by key with a
        // seen-list instead (the pending list is short).
        let mut seen_keys: Vec<_> = Vec::new();
        for pending_event in pending_list.iter().filter(|e| {
            if seen_keys.contains(&e.key) {
                false
            } else {
                seen_keys.push(e.key);
                true
            }
        }) {
...
mod tests {
            ...
            for do_cancel in [true, false] {
                ...
                assert_eq!(
                    event
                        ...
                        .unwrap(),
                    m6_starnix_std::mem::size_of::<u64>()
                );
    ...
    async fn test_cancel_after_notify() {
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            assert_eq!(
                epoll_file
                    .wait(locked, &current_task, 10, zx::MonotonicInstant::ZERO)
                    .unwrap()
                    .len(),
                1
            );
