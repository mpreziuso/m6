...
//!
//! impl<'guard, G: 'guard + m6_starnix_std::ops::Deref<Target = FooMutableState>> Guard<'guard, Foo, G> {
//!     fn x_and_y(&self) -> i32 {
//!         self.base.x + self.y
//!     }
//!     /// Some rustdoc.
...
//! }
//!
//! impl<'guard, G: 'guard + m6_starnix_std::ops::DerefMut<Target = FooMutableState>> Guard<'guard, Foo, G> {
//!     fn set_y(&mut self, other_y: i32) {
//!         self.y = other_y;
//!     }
//!     pub fn pub_set_y(&mut self, other_y: i32) {
...
macro_rules! state_implementation {
    ...
    (impl $mutable_name:ident<Base=$base_name:ident, BaseType = $base_type:ty> {
        ...
        impl<'guard, G: 'guard + m6_starnix_std::ops::Deref<Target=$mutable_name>> $crate::mutable_state::Guard<'guard, $base_type, G> {
            filter_methods_macro::filter_methods!(RoMethod, $($tt)*);
        }

        impl<'guard, G: 'guard + m6_starnix_std::ops::DerefMut<Target=$mutable_name>> $crate::mutable_state::Guard<'guard, $base_type, G> {
