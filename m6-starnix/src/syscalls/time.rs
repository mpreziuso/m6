...
pub fn sys_gettimeofday(
    ...
    if !user_tz.is_null() {
        ...
        current_task.mm()?.check_plausible(user_tz.addr(), m6_starnix_std::mem::size_of::<timezone>())?;
...
pub fn sys_settimeofday(
    ...
    // M6 minimal core: the UTC time-adjustment proxy is a gated Fuchsia
    // (FIDL `fuchsia.time.external`) subsystem; setting time is unsupported.
    ...
    match waiter.wait(locked, current_task) {
        ...
        Err(err) if err == EINTR => {
            if !user_remaining.is_null() {
                ...
                let remaining = timespec_from_duration(*m6_starnix_std::cmp::max(
...
mod arch32 {
    ...
    pub fn sys_arch32_clock_gettime64(
        ...
        const_assert!(
            m6_starnix_std::mem::size_of::<uapi::timespec>()
                == m6_starnix_std::mem::size_of::<uapi::arch32::__kernel_timespec>()
        );
    ...
    pub fn sys_arch32_timer_gettime64(
        ...
        const_assert!(
            m6_starnix_std::mem::size_of::<uapi::itimerspec>()
                == m6_starnix_std::mem::size_of::<uapi::arch32::__kernel_itimerspec>()
        );
...
mod test {
    ...
    async fn test_nanosleep_without_remainder() {
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            let address = map_memory(
                ...
                &current_task,
                UserAddress::default(),
                m6_starnix_std::mem::size_of::<timespec>() as u64,
            );
    ...
    async fn test_clock_nanosleep_interrupted_relative_to_fast_utc_clock() {
        spawn_kernel_and_run(async |locked, current_task| {
            ...
            let thread_join_handle = m6_starnix_std::thread::Builder::new()
                .name("clock_nanosleep_interruptor".to_string())
                .spawn(move || {
                    m6_starnix_std::thread::sleep(m6_starnix_std::time::Duration::from_nanos(
