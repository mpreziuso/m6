...
use crate::mm_ref::{MemoryAccessor, MemoryAccessorExt, PAGE_SIZE};
use crate::security;
...
pub fn do_uname(
    ...
    let version = current_task.kernel().build_version.get_or_try_init(|| {
        #[cfg(feature = "fuchsia")]
        {
            let proxy = connect_to_protocol_sync::<buildinfo::ProviderMarker>()
        ...
        // -- M6 has no buildinfo FIDL provider; use a static version string.
        #[cfg(not(feature = "fuchsia"))]
        Ok::<_, Errno>("starnix".to_string())
    })?;
...
pub fn sys_getrandom(
    ...
    let mut bounce_buffer = Vec::with_capacity(m6_starnix_std::cmp::min(*PAGE_SIZE as usize, size));
    let bounce_buffer = bounce_buffer.spare_capacity_mut();

    let bytes_to_write = m6_starnix_std::cmp::min(size, MAX_RW_COUNT);

    while bytes_written < bytes_to_write {
        let chunk_start = start_addr.saturating_add(bytes_written);
        let chunk_len = m6_starnix_std::cmp::min(*PAGE_SIZE as usize, size - bytes_written);
...
pub fn sys_delete_module(
    ...
    const MODULE_NAME_LEN: usize = 64 - m6_starnix_std::mem::size_of::<u64>();
