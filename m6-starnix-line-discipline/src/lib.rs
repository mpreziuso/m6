// line_discipline — Starnix-fork shim for M6 (no_std).
//
// Ported from Fuchsia `//src/starnix/lib/line_discipline`. Adapted for `no_std`:
//   * `std::{collections::VecDeque, cmp, iter, mem, ops}` -> `alloc` / `core`;
//   * the `derivative`-derived `Default`/`Debug` impls of `LineDiscipline` are
//     written by hand (to drop the proc-macro dependency);
//   * the upstream `#[cfg(test)]` Fuchsia tests and `pub mod testing` are
//     omitted (they rely on the Fuchsia test harness).
//
// The `InputBuffer`/`OutputBuffer` trait definitions are kept identical to
// upstream so that callers (e.g. the fork's `device/terminal.rs`) can implement
// them for their own buffer wrappers.
#![no_std]
...
impl Default for LineDiscipline {
    fn default() -> Self {
        Self {
            locked: true,
            stopped: false,
            window_size: uapi::winsize::default(),
            termios: get_default_termios(),
            erasing: false,
            lnext: false,
            column: 0,
            main_references: None,
            replica_references: None,
...
impl fmt::Debug for LineDiscipline {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("LineDiscipline")
            ...
            .field("erasing", &self.erasing)
            .field("lnext", &self.lnext)
            .field("column", &self.column)
            .field("main_references", &self.main_references)
            .field("replica_references", &self.replica_references)
            ...
            match character_bytes[0] {
                ...
                b'\t' => {
                    ...
                    if self.termios.c_oflag & TABDLY == XTABS {
                        self.column += spaces;
                        queue.line_buffer.extend(core::iter::repeat(b' ').take(spaces));
                        continue;
                    }
                    self.column += spaces;
                }
...
impl Queue {
    ...
    fn write(
        ...
        let buffer = data.read_to_vec_exact(core::cmp::min(room, data_length))?;
    ...
    fn flush_line_buffer(&mut self) {
        self.read_queue.push_back(core::mem::take(&mut self.line_buffer));
    }
...
fn generate_erase_echo(erase_span: &BufferSpan) -> Vec<RawByte> {
    ...
    erase_echo.iter().cycle().take(erase_echo.len() * erase_span.characters).copied().collect()
}
...
impl core::ops::AddAssign<Self> for BufferSpan {
