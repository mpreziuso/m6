// Forked from Fuchsia's starnix_registers (aarch64), adapted for M6 (no_std).
// Original: Copyright 2023 The Fuchsia Authors. BSD-style license.

use crate::{
    HeapRegs, RegisterStorage, RegisterStorageEnum, ZX_REG_CPSR_ARCH_32_MASK,
    ZX_REG_CPSR_THUMB_MASK,
};
use core::fmt;
...
///
/// Implements [`core::ops::Deref`] and [`core::ops::DerefMut`] as a way to get at the underlying
...
impl<T: RegisterStorage> ArchSpecific for RegisterState<T> {
    fn is_arch32(&self) -> bool {
        (self.cpsr as u64) & ZX_REG_CPSR_ARCH_32_MASK == ZX_REG_CPSR_ARCH_32_MASK
    }
}

impl<T: RegisterStorage> RegisterState<T> {
    fn is_thumb(&self) -> bool {
        const IS_THUMB_MASK: u64 = ZX_REG_CPSR_ARCH_32_MASK | ZX_REG_CPSR_THUMB_MASK;
    ...
    pub fn set_instruction_pointer_register(&mut self, mut new_ip: u64) {
        if self.is_arch32() {
            ...
            if is_thumb {
                new_ip -= 1;
                self.cpsr |= ZX_REG_CPSR_THUMB_MASK as u32;
            } else {
                self.cpsr &= !ZX_REG_CPSR_THUMB_MASK as u32;
            }
            self.r[15] = new_ip;
    ...
    /// Rewind the register that indicates the instruction pointer by one syscall instruction.
    ...
    pub fn reset_flags(&mut self) {
        ...
        self.cpsr &= (ZX_REG_CPSR_ARCH_32_MASK | ZX_REG_CPSR_THUMB_MASK) as u32;
    }
    ...
    pub fn apply_user_register(
        ...
        if offset >= core::mem::size_of::<user_regs_struct>() {
            return error!(EINVAL);
        }
            ...
            || (offset == reg_offset(15) && is_arch32)
        {
            ...
            if is_arch32 {
                // The 14th register is stored as lr in self.real_registers for arm
                self.r[14] = self.r[30];
            }
        } else if offset.is_multiple_of(LongPtr::align_of_object_for(self)) {
    ...
    pub fn sync_stack_ptr(&mut self) {
        ...
        if self.cpsr & ZX_REG_CPSR_ARCH_32_MASK as u32 != 0 {
            self.sp = self.r[13];
            self.r[30] = self.r[14];
...
impl<T: RegisterStorage> fmt::Debug for RegisterState<T> {
...
impl<T: RegisterStorage> core::ops::Deref for RegisterState<T> {
...
impl<T: RegisterStorage> core::ops::DerefMut for RegisterState<T> {
