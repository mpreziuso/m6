...
//! A sorted interval map (`RangeMap`) backed by a copy-on-write B-tree.
//!
//! Ported verbatim from Fuchsia's `//src/starnix/lib/range_map`, adapted to
//! `no_std` by sourcing `Vec`/`Arc` from `alloc` and the remaining items from
//! `core`.

#![no_std]
// Ported verbatim from upstream; keep the original structure rather than
// applying clippy's stylistic suggestions.
