...
use m6_cap::{Badge, CNodeOps, CapRights, CdtNodeId, RevocationCallback, ops};
use m6_syscall::IpcBuffer;

use crate::cap::cdt_storage;
use crate::cap::cspace::{self, CNodeAccess, with_two_cnodes};
use crate::cap::object_table::{self, KernelObjectType};
use crate::syscall::SyscallArgs;
use crate::syscall::error::{SyscallError, SyscallResult, cap_error_to_syscall};
...
pub fn handle_cap_move(args: &SyscallArgs) -> SyscallResult {
    ...
    // Perform the move
    with_two_cnodes(src_loc.cnode_ref, dest_loc.cnode_ref, |access| {
        match access {
            CNodeAccess::Distinct(src_cnode, dest_cnode) => {
                ops::cap_move(
                    src_cnode,
                    src_loc.slot_index,
                    dest_cnode,
            ...
            CNodeAccess::Same(cnode) => {
                ops::cap_move_local(cnode, src_loc.slot_index, dest_loc.slot_index)
                    .map_err(cap_error_to_syscall)?;
            }
        }
