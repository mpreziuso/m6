// Forked from Fuchsia's Starnix for M6.
// Original: Copyright 2025 The Fuchsia Authors. BSD license.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.
//
// This is a proc-macro crate and is therefore host-compiled; the `no_std`
// nature of the target does not apply here. The generated token streams emit
// `::core::` paths (rather than upstream's `std::`) so that expansions compile
// inside the `no_std` Starnix fork.
...
fn generate_container(input: syn::ItemStruct) -> syn::Result<TokenStream> {
    ...
    for field in &named_fields.named {
        ...
        if contains_container_attrs {
            ...
            updated_fields.push(quote!(#payload_field_name: ::core::mem::ManuallyDrop<#payload_ty>));
...
fn generate_split_storage_impl_inner(input: syn::DeriveInput) -> syn::Result<TokenStream> {
        ...
        match &variant.fields {
            ...
            syn::Fields::Unnamed(fields) => {
                ...
                payload_fields.push(quote! {
                    #payload_field_name: ::core::mem::ManuallyDrop<#field_type>
                });
                ...
                decompose_arms.push(quote! {
                    #ident::#variant_name(inner) => (
                        #discriminant_name::#variant_name,
                        #payload_name { #payload_field_name: ::core::mem::ManuallyDrop::new(inner) },
                    )
                });
                clone_arms.push(quote! {
                ...
                read_arms.push(quote! {
                    #discriminant_name::#variant_name => #ident::#variant_name(
                        ::core::mem::ManuallyDrop::into_inner(self.#payload_field_name.clone())
                    )
                });
                read_ref_arms.push(quote! {
                    #discriminant_name::#variant_name => #ref_name::#variant_name(
                ...
                free_arms.push(quote! {
                    #discriminant_name::#variant_name =>
                        ::core::mem::ManuallyDrop::drop(&mut self.#payload_field_name)
                });
            }
    ...
    let expanded = quote! {
        ...
        unsafe impl split_enum_storage::SplitStorage for #ident {
            ...
            fn decompose(self) -> (#discriminant_name, ::core::mem::ManuallyDrop<#payload_name>) {
                let (discriminant, payload) = match self {
                    #(#decompose_arms),*
                };
                (discriminant, ::core::mem::ManuallyDrop::new(payload))
            }
        }
        ...
        impl #payload_name {
            ...
            /// creation.
            unsafe fn clone(&self, disc: #discriminant_name) -> ::core::mem::ManuallyDrop<Self> {
                ::core::mem::ManuallyDrop::new(match disc {
                    #(#clone_arms),*
                })
            }
        ...
        impl ::core::ops::Drop for #payload_name {
            fn drop(&mut self) {
                ::core::panic!(#drop_message);
            }
        }
    };
