...
impl SigSet {
    ...
    pub fn has_signal(&self, signal: Signal) -> bool {
        (self.0 & (signal.mask() as c_ulong)) != 0
    }
...
impl From<sigset_t> for SigSet {
    fn from(value: sigset_t) -> Self {
        // SAFETY: `transmute()` is safe here because this is a POD value of the same size (see
        // the `assert_eq_size!` above).
        SigSet(unsafe { core::mem::transmute::<sigset_t, c_ulong>(value) })
    }
}
...
impl From<SigSet> for sigset_t {
    fn from(val: SigSet) -> Self {
        // SAFETY: `transmute()` is safe here because this is a POD value of the same size (see
        // the `assert_eq_size!` above).
        unsafe { core::mem::transmute::<c_ulong, sigset_t>(val.0) }
    }
}
