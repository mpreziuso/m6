...
/// all zeroes.
// M6: std:: -> core:: throughout for no_std.
#[macro_export]
    ...
    ($ty:ty { $($($field:ident).*: $value:expr,)* }) => {{
        ...
        $({
            ...
            if false {
                ...
                // use `core::ptr::write`, not normal field assignment, as the latter would
                ...
                unsafe {
                    core::ptr::write(&mut (&mut *(&mut bytes.bytes as *mut [u8; BYTES] as *mut $ty)).$($field).*, value);
                }
            }
        })*
        ...
        paste::paste! {
            ...
            $vis type [<$wrapper Ptr>] =
                starnix_uapi::user_address::MappingMultiArchUserRef<
                    $wrapper,
                    [u8; core::mem::size_of::<starnix_uapi::$ty>()],
                    [u8; core::mem::size_of::<starnix_uapi::arch32::$ty>()]>;
            ...
            impl From<[u8; core::mem::size_of::<starnix_uapi::$ty>()]> for $wrapper {
                fn from(bytes: [u8; core::mem::size_of::<starnix_uapi::$ty>()]) -> Self {
                    Self([<$wrapper Inner>]::Arch64(zerocopy::transmute!(bytes)))
                }
            }

            #[cfg(target_arch = "aarch64")]
            impl From<[u8; core::mem::size_of::<starnix_uapi::arch32::$ty>()]> for $wrapper {
                fn from(bytes: [u8; core::mem::size_of::<starnix_uapi::arch32::$ty>()]) -> Self {
                    Self([<$wrapper Inner>]::Arch32(zerocopy::transmute!(bytes)))
                }
            }

            impl TryFrom<$wrapper> for [u8; core::mem::size_of::<starnix_uapi::$ty>()] {
                type Error = ();
            ...
            #[cfg(target_arch = "aarch64")]
            impl TryFrom<$wrapper> for [u8; core::mem::size_of::<starnix_uapi::arch32::$ty>()] {
                type Error = ();
                fn try_from(v: $wrapper) -> Result<Self, ()> {
            ...
            impl core::fmt::Debug for $wrapper {
                fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> Result<(), core::fmt::Error> {
                    let mut s = if self.is_arch32() {
                        f.debug_tuple(core::stringify!([<$wrapper 32>]))
                    } else {
                        f.debug_tuple(core::stringify!([<$wrapper 64>]))
                    };
                    // SAFETY: All union used by this wrapper are generated from bytes, so
                    // there is no uninitialized data.
                    match self.inner () {
                        [<$wrapper Inner>]::Arch64(v) => {
                            let bytes: &[u8; core::mem::size_of::<starnix_uapi::$ty>()] =
                                unsafe { core::mem::transmute(v) };
                            s.field(bytes);
                        }
                        [<$wrapper Inner>]::Arch32(v) => {
                            let bytes: &[u8; core::mem::size_of::<starnix_uapi::arch32::$ty>()] =
                                unsafe { core::mem::transmute(v) };
                            s.field(bytes);
                        }
                    }
